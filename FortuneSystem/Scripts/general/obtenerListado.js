    ...
    var comentario = $("#PrintShopC_Comentarios").val();
      if (error !== 0) {
        ...
        $.ajax({
            ...
            datatType: 'json',
            data: JSON.stringify({ ListTalla: cadena, EstiloID: estiloId, MaquinaID: idMaquina, StatusID: idStatus, Comentarios: comentario }),
            cache: false,
            type: 'POST',
...
function obtener_tallas_item(IdEstilo/*, numUnits*/) {
    var tempScrollTop = $(window).scrollTop(); 
    ...
    $.ajax({
      ...
      //  data: "{'id':'" + IdEstilo + "','numUnits':'" + numUnits + "'}",
        ...
        success: function (jsonData) {
            ...
            html += '</tr><tr><td>PNL Quantity <button type="button" class="btn btn-primary btn-sm" Title = "Details PNL" onclick="TablaPNL(' + IdEstilo + ');"><span class="glyphicon glyphicon-pushpin" aria-hidden="true" style="padding: 0px !important;"></span> </button></td>';
            var cantidadesPNL = 0;
            var lista_Batch_PNL = jsonData.Data.listaTallasTotalBatchPnl;
    ...
    $.ajax({
        ...
        success: function (jsonData) {
            ...
            $.each(lista_batch, function (key, item) {
                ...
                if (item.NombreUsr === null && item.NombreUsr === "") {
                    html += '<td>' + - + '</td>';
                } else {
    ...
    $.ajax({
        ...
        success: function (jsonData) {
            ...
            $.each(lista_estilo_Tallas, function (key, item) {
                ...
                item.Repair = 0;
                html += '<td ><input type="text" id="repair' + contadorR + '" class="txt form-control rep numeric " onfocus="focusingR(' + contadorR +')" onChange="calcular_Repair()" value="' + item.Repair + '"/></td>';
            ...
            $.each(lista_estilo_Tallas, function (key, item) {
                html += ' <div class="span7">';              
                    item.Defect = item.Talla;
                    item.Defect = 0;
                html += '<td class="cRepa"><input type="text" id="falt" class="form-control number totalFal" value="' + item.Defect + '" readonly/></td>';
